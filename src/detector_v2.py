...
import sys
sys.path.append("../..")
import src.KR_norm_juicer as KRnorm
...
class TADDetector:
    ...
    def __init__(self, input_matrix, step_scal=1, KR_norm=False, acc_mode=False):
        ...
        self.dp_table = np.zeros(shape=(self._N_, self.l_max+1))
        # self.dp_table = []
        self.table_i = np.zeros(shape=(self._N_, self.l_max+1), dtype=int)
        self.table_t = np.zeros(shape=(self._N_, self.l_max+1), dtype=int)
    ...
    def dp_process_one_layer(self):
        for j in range(self._N_):
            ...
            if abs(t_c - int(t_c)) <= abs(int(t_c) + 1 - t_c):
                ...
                t = int(t_c) + 1
            self.dp_table[j, t] = H_t
        for i_r in range(1, self._N_):
            dp_where = np.where(self.dp_table[:i_r, :] != float("inf"))
            for dp_i in range(len(dp_where[0])):
                ...
                if t is None:
                    continue
                H_i_km1 = self.dp_table[i, t_ac]
                tmp = H_i_km1 + self.cal_delta((i + 1), i_r)
                if tmp < self.dp_table[i_r, t]:
                    self.dp_table[i_r, t] = tmp
                    self.table_i[i_r, t] = i
                    self.table_t[i_r, t] = t_ac
            del dp_where
        print(self.dp_table)
    ...
    def find_all_nodes_with_tables(self, t_f):
        bound = ([], [])
        ...
        t_tmp = t_f
        i_r = self._N_ - 1
        while find_all==False:
            bound = self.bound_list[list_start]
            if k != 0:
                i_r = bound[0][-1] - 1
            bound[1].append(i_r)
            i_pre = self.table_i[i_r, t_tmp]
            t_pre = self.table_t[i_r, t_tmp]
            ...
            if i_pre>self._N_ or t_pre>self.l_max:
                break
            t_tmp = t_pre
            i_r = i_pre
    ...
    # find the optimal boundary
    def backtrace_H(self):
        H_index = np.where(self.dp_table[self._N_ - 1, :] != float("inf"))
        ...
        #     H_tmp = self.dp_table[self._N_ - 1, t]
        ...
        # for i in range(len(min_t)):
            # ***********************************Compute the accurate values
            self.find_all_nodes_with_tables(t_f=t)
            #   ***************************Compute the estimated value *****************************************************
            # print("step_scal: "+str(self.step_scal)+", step_size: "+str(self.step_size)+", infomap_estimated: "+str(infomap_min)+", bound: "+str(self.bound_list[i]))
            # with open("entro_estimated_log.txt", 'a') as f:
            #     content = "step_scal: "+str(self.step_scal)+", step_size: "+str(self.step_size)+", infomap_estimated: "+str(infomap_min)+", bound: "
            #     f.write(content)
            # with open("entro_estimated_log.txt", 'ab') as f:
            #     np.savetxt(f, self.bound_list[i], fmt='%i')
            #   ***************************Compute the estimated value *****************************************************
        self.reverse_bound()
        tree_num = len(self.bound_list)
        entro_list = []
    ...
    def output_bounds(self, opt_bound_list, data_file_name, start_pos=1):
        ...
        output = []
        match = re.search(r'chr(\d+)_', data_file_name)
        ...
        for i in range(len(opt_bound_list)):
            ...
            for b_i in range(len(bound[0])):
                line = [chr_name, int(bound[0][b_i] + start_pos), int(bound[0][b_i] + start_pos - 1) * resolution, int(bound[0][b_i] + start_pos) * resolution,
                        chr_name, int(bound[1][b_i] + start_pos), int(bound[1][b_i] + start_pos - 1) * resolution, int(bound[1][b_i] + start_pos) * resolution]
                output.append(line)
        # write file
        output = DataFrame(output)
        with open("../temp.tsv", 'w') as write_tsv:
    ...
    def construct_one_layer(self, filename="simulate_data", start_pos=1):
        ...
        opt_bound_list = self.backtrace_H()
