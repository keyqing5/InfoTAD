...
def KRnorm_asym(test, verbose=False, oneNorm=False):
    ...
    normMatrix = test.copy()
    for i in range(length_r):
        for j in range(length_c):
            if not (np.isnan(result_r[i])) and not (np.isnan(result_c[j])):
                normMatrix[i, j] = normMatrix[i, j] / (result_r[i] * result_c[j])
    return normMatrix, result

def KRnorm_sym(test, verbose=False, norm=False):
    result = Compute_KR(test, verbose=verbose, Onenorm=norm).run()
    length = len(test)
    normMatrix = test.copy()
    for i in range(length):
        for j in range(i, length, 1):
            if (not (np.isnan(result[i])) and not (np.isnan(result[j]))):
                normMatrix[i, j] = normMatrix[i, j] / (result[i] * result[j])
                normMatrix[j, i] = normMatrix[i, j]
    return normMatrix, result
    ...
    def computeKR(self):
        ...
        while recalculate and iteration <= max_iter:
            ...
            if x0 is None or iteration == max_iter - 1:
                ...
                if self.verbose:
                    print(iteration, kr)
                    print("nan has {}, zero has {}".format(len(np.where(np.isnan(kr) == True)[0]),
                                                           len(np.where(kr == 0)[0])))
    ...
    def getSumFactor(self, norm_list):
        ...
        for i in range(self.length):
            for j in range(i, self.length, 1):
                if (not (np.isnan(norm_list[i])) and not (np.isnan(norm_list[j])) and norm_list[i] > 1e-36 and
                        norm_list[j] > 1e-36):  # only if the factor is larger than 0, valid
